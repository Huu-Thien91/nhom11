...
export default {
  ...
  methods: {
    ...
    openEditEpisodeModal(episode) {
      this.editingEpisode = { ...episode }; // Copy episode data to edit
      this.showEditEpisodeModal = true;
    },
    async updateEpisode() {
      try {
        const formData = new FormData();
        formData.append("EpisodeId", this.editingEpisode.episodeId);
        formData.append("EpisodeNumber", this.editingEpisode.episodeNumber);
        formData.append("Title", this.editingEpisode.title);
        formData.append("LinkFilmUrl", this.editingEpisode.linkFilmUrl);

        const response = await axios.post("http://localhost:5289/api/AdminEpisode/UpdateEpisode", formData, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        ...
        if (response.status === 200) {
          ...
          const series = this.seriesList.find(s => s.seriesId === this.selectedSeriesId);
          if (series) {
            const index = series.episodes.findIndex(e => e.episodeId === updatedEpisode.episodeId);
            if (index !== -1) {
              series.episodes.splice(index, 1, updatedEpisode); // Cập nhật tập phim
            }
          }
    ...
    closeEditEpisodeModal() {
      this.showEditEpisodeModal = false;
      this.editingEpisode = {}; // Clear the editing object
    },
